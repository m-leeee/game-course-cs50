...
public class LevelGenerator : MonoBehaviour {
	...
	private int holerand = 0;
	private int iters = 0;
	public static int roomcount;

	// Use this for initialization
	void Start () {
		holerand = Random.Range(0,5);
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				if (mapData[z, x]) {
					...
					if ((holecount < 5) && (iters >= holerand) && (Random.Range(1,3)>=2))
					{
						holerand += Random.Range(mazeSize*mazeSize/8-3,mazeSize*mazeSize/8+3);
