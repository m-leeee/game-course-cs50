...
function Room:update(dt)
    ...
    for i = #self.entities, 1, -1 do
        ...
        if entity.health <= 0 then
            ...
            if entity.drops == false then
                if math.random(2) == 1 then --this keeps checking in update while the entity is dead 
                    --drop a heart
                    --local heart = GameObject(GAME_OBJECT_DEFS['switch'], entity.x, entity.y)
                    --table.insert(self.objects, heart)
                    self:generateHeart(entity.x,entity.y,self.player)
                end
                entity.drops = true
            end
